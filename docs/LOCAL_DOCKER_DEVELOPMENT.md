...
## AI Summary Local Checklist

1. Add Azure OpenAI settings to `.env.local`.
2. Rebuild the local stack so `case-api` receives the new environment variables.
3. Ensure `cosmos-seeder` has created the `casemanagement/aiusagequotas` container.
