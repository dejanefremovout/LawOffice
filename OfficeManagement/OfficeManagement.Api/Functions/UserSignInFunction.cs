...
/// On first sign-in, creates the office and lawyer record using the OfficeName
/// extension attribute read from Microsoft Graph.
/// </summary>
public class UserSignInFunction(ILawyerService lawyerService,
    ...
    IGraphUserService graphUserService)
{
    ...
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "usersignin")] HttpRequest req)
    {
        ...
        if (!string.IsNullOrWhiteSpace(userEmail))
        {
            ...
            // First sign-in: read OfficeName from Graph extension attribute
            // and create the office + lawyer record.
            var officeName = await _graphUserService.GetOfficeName(userEmail);
            ...
            var office = await _officeService.Create(new OfficeCreateModel { Name = officeName });

            var displayName = GetDisplayName(calloutData) ?? userEmail;

            var newLawyer = await _lawyerService.Create(new LawyerCreateModel
            {
            ...
            return BuildContinueResponse(newLawyer.OfficeId);
        }
    ...
    private static string? GetDisplayName(JsonElement calloutData)
    {
        if (calloutData.TryGetProperty("authenticationContext", out var authCtx)
            && authCtx.TryGetProperty("user", out var ctxUser)
            && ctxUser.TryGetProperty("displayName", out var dn1)
            && dn1.ValueKind == JsonValueKind.String)
        {
            return dn1.GetString();
        }

        if (calloutData.TryGetProperty("user", out var user)
            && user.TryGetProperty("displayName", out var dn2)
            && dn2.ValueKind == JsonValueKind.String)
        {
